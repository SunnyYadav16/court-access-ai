...
def decode_refresh_token(token: str) -> TokenPayload:
    """
    Decode and validate a refresh JWT and return its parsed payload.
    
    Returns:
        TokenPayload: Parsed token payload containing `sub`, `role`, `type`, `jti`, and `exp` (as a UTC datetime).
    """
...
def verify_firebase_token(token: str) -> dict[str, Any]:
    """
    Verify a Firebase ID token and return the decoded Firebase claims.
    
    Parameters:
        token (str): Firebase ID token (typically from the Authorization header).
    
    Returns:
        dict[str, Any]: Decoded Firebase token claims (e.g., `uid`, `email`, `email_verified`, `name`, `firebase.sign_in_provider`, `iat`, `exp`, ...).
    
    Raises:
        HTTPException: 401 Unauthorized with one of these details:
            - "Token expired" when the token has expired.
            - "Token revoked" when the token was revoked.
            - "Invalid token" when the token is malformed or invalid.
            - "Account disabled" when the Firebase user account is disabled.
            - "Authentication failed" for other verification errors.
    """
    try:
...
async def get_or_create_user(claims: dict[str, Any], db: AsyncSession):
    """
    Finds a User by Firebase UID and updates their last-login/email verification or creates a new User from Firebase claims.
    
    Parameters:
        claims (dict[str, Any]): Decoded Firebase ID token claims. Expected keys include "uid" and optionally "email", "name", "email_verified", and a nested "firebase" object with "sign_in_provider".
        db (AsyncSession): Async SQLAlchemy session used to query and persist the User record.
    
    Returns:
        User: The ORM User instance corresponding to the Firebase UID; existing users are updated and returned, new users are created and returned.
    """
