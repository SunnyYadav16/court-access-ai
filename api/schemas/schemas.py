...
class UserResponse(BaseModel):
    ...
    def from_orm_user(cls, user) -> UserResponse:
        """
        Create a UserResponse model populated from a User ORM instance.
        
        Parameters:
            user: ORM user object with attributes matching UserResponse fields (e.g., user_id, email, name, role_id, firebase_uid, auth_provider, email_verified, mfa_enabled, role_approved_by, role_approved_at, created_at, last_login_at).
        
        Returns:
            UserResponse: Response model with `role` set to the human-readable name looked up from ROLE_ID_TO_NAME; if the role_id is not present in the mapping, `role` is set to `"unknown"`.
        """
        return cls(
