...
class TestRunScrape:
    ...
    def test_no_change_increments_count(self):
        ...
        entry = _make_catalog_entry(
            source_url="https://mass.gov/doc/a/download", content_hash=_sha256(pdf), form_name="Form A"
        )
    ...
    def test_updated_form_increments_count(self):
        ...
        new_pdf = _make_pdf("new content")
        entry = _make_catalog_entry(source_url="https://mass.gov/doc/a/download", content_hash=_sha256(old_pdf))
        sf._save_catalog([entry])
        result = self._run(
            scraped_forms=[
                {
    ...
    def test_deleted_form_increments_count(self):
        entry = _make_catalog_entry(source_url="https://mass.gov/doc/gone/download", status="active")
        sf._save_catalog([entry])
        result = self._run(scraped_forms=[], download_map={"https://mass.gov/doc/gone/download": None})
        assert result["counts"]["deleted"] == 1

    def test_transient_error_does_not_archive(self):
        import requests as req_lib

        entry = _make_catalog_entry(source_url="https://mass.gov/doc/flaky/download", status="active")
        sf._save_catalog([entry])
        result = self._run(
            scraped_forms=[],
    ...
    def test_renamed_form_increments_count(self):
        pdf = _make_pdf("same content")
        entry = _make_catalog_entry(
            source_url="https://mass.gov/doc/old-url/download", content_hash=_sha256(pdf), form_name="Old Name"
        )
        sf._save_catalog([entry])
        result = self._run(
            scraped_forms=[
