...
/**
 * Manage a session-scoped WebSocket connection for real-time interpretation and expose send/disconnect controls.
 *
 * Establishes and maintains a WebSocket to the session's endpoint, automatically reconnects (limited retries)
 * on unexpected closes, dispatches incoming transcript and error messages to the provided callbacks, and
 * provides methods to send messages and intentionally disconnect.
 *
 * @param sessionId - The session identifier to connect to; when null or falsy the hook does nothing.
 * @param options - Optional callbacks:
 *   - onTranscript: invoked with a TranscriptSegment when a `transcript` message arrives.
 *   - onError: invoked with a WSError when an `error` message arrives or when max reconnects are exceeded.
 *   - onStatusChange: invoked whenever the connection status changes.
 * @returns An object with:
 *   - status: the current WSStatus ("idle" | "connecting" | "connected" | "reconnecting" | "disconnected" | "error").
 *   - send(type, payload): send a typed message over the active WebSocket (ignored if not open).
 *   - disconnect(): intentionally close the connection and stop any pending reconnect attempts.
 */
