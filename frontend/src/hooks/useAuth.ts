...
/**
 * Exposes authentication state, actions, modal controls, and convenience helpers from the auth store as a React hook.
 *
 * @returns An object containing:
 * - State: `user`, `authState`, `role`, `backendUser`, `isLoading`, `error`, `authModalOpen`, `authModalView`, `pendingVerificationEmail`.
 * - Auth actions: `signInWithGoogle`, `signInWithMicrosoft`, `signInWithEmail`, `createAccount`, `signOut`, `sendPasswordReset`, `resendVerificationEmail`.
 * - Modal actions: `openAuthModal`, `closeAuthModal`, `setAuthModalView`.
 * - Utilities: `clearError`.
 * - Helpers: `hasRole` (checks whether current role matches any provided roles) and `requireAuth` (runs a callback if authenticated or opens the login modal otherwise).
 */
