...
def task_load_form_entry(**context) -> dict:
    ...
    if not Path(orig_path).exists():
        raise FileNotFoundError(
            f"Original PDF not found on disk: '{orig_path}'. The scraper may not have saved it yet."
        )
    ...
    if already_has_es and already_has_pt:
        logger.info(
            "Form '%s' already has ES and PT translations from mass.gov. Skipping — nothing for this DAG to do.",
            form_id,
        )
...
def task_reconstruct_pdf_spanish(**context) -> dict:
    ...
    if form_meta.get("skip") or form_meta.get("already_has_es"):
        ...
        ti.xcom_push(
            key="path_es", value=form_meta["versions"][0]["file_path_es"] if not form_meta.get("skip") else None
        )
        return {}
    ...
    try:
        ...
        logger.warning(
            "Spanish PDF reconstruction skipped — original is not a valid PDF: %s. "
            "Form will be flagged for human review.",
            exc,
        )
...
def task_reconstruct_pdf_portuguese(**context) -> dict:
    ...
    try:
        ...
        logger.warning(
            "Portuguese PDF reconstruction skipped — original is not a valid PDF: %s. "
            "Form will be flagged for human review.",
            exc,
        )
        ti.xcom_push(key="path_pt", value=None)
...
def task_store_and_update_catalog(**context) -> dict:
    ...
    logger.info(
        "Catalog updated for '%s': file_path_es=%s, file_path_pt=%s, languages_available=%s, needs_human_review=True.",
        form_id,
        path_es,
        path_pt,
        langs,
...
def task_dvc_version_data(**context) -> None:
    ...
    def _run(cmd: list[str]) -> bool:
        try:
            res = subprocess.run(  # noqa: S603 — cmd is always a hardcoded DVC list, never user input
                cmd,
                cwd=PROJECT_ROOT,
                capture_output=True,
                text=True,
