...
def upgrade() -> None:
    """
    Add Firebase authentication fields and related constraints to the users table.
    
    Adds columns: `firebase_uid`, `auth_provider`, `email_verified`, `mfa_enabled`,
    `name`, `role_approved_by`, and `role_approved_at`. Removes temporary server
    defaults for `firebase_uid` and `auth_provider` after backfill, makes
    `hashed_password` nullable, creates indexes `ix_users_firebase_uid`
    (unique) and `ix_users_auth_provider`, and adds foreign key constraint
    `fk_users_role_approved_by` referencing `users.user_id` with ON DELETE SET NULL.
    """
    # Add new columns
    op.add_column(
        "users",
...
def downgrade() -> None:
    """
    Revert migration changes: remove Firebase-related columns, indexes, and foreign key from the users table and restore hashed_password to non-nullable.
    
    This drops the foreign key constraint from users.role_approved_by, removes indexes on firebase_uid and auth_provider, sets users.hashed_password back to non-nullable, and drops the columns added for Firebase integration (firebase_uid, auth_provider, email_verified, mfa_enabled, name, role_approved_by, role_approved_at).
    """
    # Drop foreign key
