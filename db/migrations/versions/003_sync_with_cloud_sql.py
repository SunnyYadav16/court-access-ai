...
def upgrade() -> None:
    """
    Synchronize the local database schema with the Cloud SQL schema by dropping existing local tables and recreating tables to match Cloud SQL exactly.
    
    This migration drops existing tables (using CASCADE) and recreates roles (including seed data), users, sessions, translation_requests, audit_logs, form_catalog, form_versions, and form_appearances in an order that preserves foreign-key dependencies.
    """
...
def downgrade() -> None:
    """
    Remove all tables created by this migration in dependency-safe order.
    
    Drops the following tables in order: form_appearances, form_versions, form_catalog, audit_logs, translation_requests, sessions, users, roles.
    """
